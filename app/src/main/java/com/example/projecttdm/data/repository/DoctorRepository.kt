...
class DoctorRepository {
    ...
    fun getDetailDoctorById(doctorId: String): Doctor {
        // Try to find the doctor in the existing list first
        val existingDoctor = getDoctorById(doctorId)

        // If found, return that doctor
        if (existingDoctor != null) {
            return existingDoctor
        }

        // Fallback doctor data if ID not found
        return Doctor(
            id = "dr-jenny-watson",
    ...
    // Check if a doctor is in a patient's favorites
    ...
    // Initialize current session favorites with the default patient's favorites
    init {
